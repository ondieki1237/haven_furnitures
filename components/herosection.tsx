...
import { ArrowRight, Shield, Award, Heart, Eye, ShoppingCart } from "lucide-react";
    ...
    <section className="relative min-h-screen bg-gradient-to-br from-[#f9f6f2] via-[#fefcf9] to-[#e8e2d9] overflow-hidden flex items-center">
      ...
      <div className="relative z-10 container mx-auto px-6 lg:px-12 py-20 flex flex-col lg:flex-row items-center gap-16">
        ...
        <div className="flex-1 space-y-10 max-w-2xl">
          ...
          >
            ...
            <h1 className="font-serif font-bold text-[#2d1a06] leading-tight tracking-tight text-5xl sm:text-6xl lg:text-7xl drop-shadow-sm">
              Redefine <span className="text-[#8b5a2b]">Luxury</span>,  
              Embrace <span className="italic">Comfort</span>
            </h1>

            <p className="text-lg sm:text-xl text-[#5c4a36] max-w-lg leading-relaxed font-sans opacity-90">
              Discover furniture that blends timeless elegance with modern
              comfort. Each piece is crafted to elevate your home into a
        ...
        <motion.div
          ...
          className="flex-1 flex justify-center relative max-w-2xl"
        >
          {/* Main Featured Product */}
          <div className="relative">
            <motion.div
              className="relative bg-white/80 backdrop-blur-xl rounded-3xl p-6 shadow-2xl border border-[#e7e1d6]/60 w-80 h-96 group cursor-pointer"
            ...
            >
              ...
              <div className="absolute bottom-6 left-6 right-6 bg-white/95 backdrop-blur-sm rounded-xl p-4 shadow-lg transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                <h3 className="font-semibold text-[#2d1a06]">{featuredProducts[0].name}</h3>
                <p className="text-sm text-[#5c4a36]/70">{featuredProducts[0].category}</p>
                <p className="text-lg font-bold text-[#8b5a2b]">{featuredProducts[0].price}</p>
              </div>
            </motion.div>
              ...
              >
                <div className="flex gap-3 h-full">
                  ...
                  <div className="flex-1 flex flex-col justify-center">
                    <h4 className="font-semibold text-sm text-[#2d1a06] leading-tight">{product.name}</h4>
                    <p className="text-xs text-[#5c4a36]/70">{product.category}</p>
                    <p className="text-sm font-bold text-[#8b5a2b] mt-1">{product.price}</p>
                  </div>
                </div>
              </motion.div>
            ))}
            ...
            <motion.div
              ...
              className="absolute -right-20 top-1/2 transform -translate-y-1/2 bg-gradient-to-br from-[#8b5a2b] to-[#6f4822] text-white p-6 rounded-2xl shadow-xl"
            >
            ...
            <motion.div
              ...
              className="absolute -left-20 bottom-1/4 bg-[#f5f0eb] text-[#8b5a2b] p-6 rounded-2xl shadow-xl border border-[#8b5a2b]/20"
            >
              <div className="text-center">
